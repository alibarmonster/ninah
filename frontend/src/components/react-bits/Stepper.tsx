...
interface StepperProps extends HTMLAttributes<HTMLDivElement> {
  ...
  /** Callback to check if current step is complete and user can proceed */
...
export default function Stepper({
  ...
  const handleNext = () => {
    // Check if we can proceed (if canProceed callback is provided)
          ...
          <div className={`px-8 pb-8 ${footerClassName}`}>
            <div className={`mt-10 flex ${currentStep !== 1 ? 'justify-between' : 'justify-end'}`}>
              {currentStep !== 1 && (
                <button
                  onClick={handleBack}
                  className={`duration-350 rounded px-2 py-1 transition font-poppins ${
                    currentStep === 1
