...
contract NinjaRupiah is ReentrancyGuard, Ownable {
    IMockIDRX public immutable idrx;

    address public sp1Verifier;

    // Base Sepolia V5.0.0 Groth16 verifier (for reference, mock uses MockSP1Verifier)
    address public constant BASE_SEPOLIA_SP1_VERIFIER = 0x50ACFBEdecf4cbe350E1a86fC6f03a821772f1e5;
    ...
    event UsernameRegistered(bytes32 indexed usernameHash, address indexed user, bytes32 commitment);

    event MetaKeysRegistered(address indexed user, bytes metaViewingPub, bytes metaSpendingPub);

    event StealthPaymentSent(
        address indexed stealthAddress, address indexed sender, uint256 amount, bytes32 ephemeralPubkeyHash
    );

    event StealthPaymentClaimed(address indexed stealthAddress, address indexed claimer, uint256 amount);
    ...
    function RegisterUsername(bytes32 usernameHash, bytes32 commitment, bytes calldata proof) external {
    ...
    function registerMetaKeys(bytes calldata metaViewingPub, bytes calldata metaSpendingPub) external {
        ...
        userMetaKeys[msg.sender] =
            MetaKeys({metaViewingPub: metaViewingPub, metaSpendingPub: metaSpendingPub, registered: true});

        emit MetaKeysRegistered(msg.sender, metaViewingPub, metaSpendingPub);
    }

    function sendToStealth(address stealthAddress, uint256 amount, bytes calldata ephemeralPubkey)
        external
        nonReentrant
    {
        ...
        emit StealthPaymentSent(stealthAddress, msg.sender, amount, ephemeralPubkeyHash);
    }

    /// @notice Claim funds from a stealth address with ZK proof
    /// @dev In mock mode, the MockSP1Verifier always returns success
    function claimFromStealth(address stealthAddress, bytes calldata proof) external nonReentrant {
        ...
        if (!verifyClaimingProof(stealthAddress, msg.sender, payment.ephemeralPubkey, proof)) {
            revert InvalidProof();
        }
    ...
    function getStealthPayment(address stealthAddress) external view returns (StealthPayment memory) {
    ...
    function isUsernameHashAvailable(bytes32 usernameHash) external view returns (bool) {
    ...
    function verifyUsernameProof(bytes32 usernameHash, bytes32 commitment, bytes calldata proof)
        internal
        view
        returns (bool)
    {
        // Decode proof data
        (bytes32 vkeyHash, bytes memory publicValues, bytes memory proofBytes) =
            abi.decode(proof, (bytes32, bytes, bytes));

        // Verify the proof via SP1 verifier (MockSP1Verifier always succeeds)
        try ISP1Verifier(sp1Verifier).verifyProof(vkeyHash, publicValues, proofBytes) {
            ...
            (bytes32 usernameHashFromProof, bytes32 commitmentFromProof) = abi.decode(publicValues, (bytes32, bytes32));
    ...
    function verifyClaimingProof(
        ...
        try ISP1Verifier(sp1Verifier).verifyProof(vkeyHash, publicValues, proofBytes) {
            ...
            (address stealthAddrFromProof, bytes32 ephemeralHashFromProof, address claimerFromProof) =
